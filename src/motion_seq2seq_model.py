...
class MoSeq2SeqModel(object):
  ...
  def __init__(self,
    ...
    print( "No. of frames per time step are ", self.FRAMES_PER_STEP)
    ...
    with tf.name_scope("inputs"):
      ...
      enc_in = tf.placeholder(dtype, shape=[None, source_seq_len - self.FRAMES_PER_STEP, self.input_size], name="enc_in")    # Ex: size: (None, 200, 54)
      dec_in = tf.placeholder(dtype, shape=[None, target_seq_len, self.input_size], name="dec_in")    # Ex: size: (None, 400, 54)
      dec_out = tf.placeholder(dtype, shape=[None, target_seq_len, self.input_size], name="dec_out")  # Ex: size: (None, 400, 54)

      self.encoder_inputs = enc_in
      self.decoder_inputs = dec_in
      self.decoder_outputs = dec_out
      
      enc_in = tf.reshape(enc_in, [-1, (source_seq_len // self.FRAMES_PER_STEP) - 1, self.FRAMES_PER_STEP, 18, 3]) # Size: (None, 50, 4, 18, 3)
      dec_in = tf.reshape(dec_in, [-1, target_seq_len // self.FRAMES_PER_STEP, self.FRAMES_PER_STEP, 18, 3]) # Size: (None, 100, 4, 18, 3)

      enc_in = tf.transpose(enc_in, [1, 0, 3, 4, 2])      # Shape: (50, None, 18, 3, 4)
      dec_in = tf.transpose(dec_in, [1, 0, 3, 4, 2])      # Shape: (100, None, 18, 3, 4)
      dec_out = tf.transpose(dec_out, [1, 0, 2])          # Shape: (400, None, 54)
      
      enc_in = tf.reshape(enc_in, [-1, 18, 3, self.FRAMES_PER_STEP])         # Shape: (50 * None, 18, 3, 4)
      dec_in = tf.reshape(dec_in, [-1, 18, 3, self.FRAMES_PER_STEP])         # Shape: (100 * None, 18, 3, 4)
      dec_out = tf.reshape(dec_out, [-1, self.input_size])# Shape: (400 * None, 54)
      
      # ==== Encoder CONV2D Layer ==== #
      W_enc = tf.get_variable('kernel', [18, 3, self.FRAMES_PER_STEP, self.rnn_size], initializer = tf.contrib.layers.xavier_initializer())
      enc_in = tf.nn.conv2d(enc_in, W_enc, strides = [1, 1, 1, 1], padding = 'VALID') # Shape: (50 * None, 1, 1, rnn_size)
      dec_in = tf.nn.conv2d(dec_in, W_enc, strides = [1, 1, 1, 1], padding = 'VALID') # Shape: (100 * None, 1, 1, rnn_size)
      enc_in = tf.squeeze(enc_in, axis = [1, 2]) # Shape: (50 * None, rnn_size)
      dec_in = tf.squeeze(dec_in, axis = [1, 2]) # Shape: (100 * None, rnn_size)
      
      enc_in = tf.split(enc_in, (source_seq_len // self.FRAMES_PER_STEP) - 1, axis = 0)  # Shape: [(None, rnn_size)] * 50
      dec_in = tf.split(dec_in, target_seq_len // self.FRAMES_PER_STEP, axis = 0)  # Shape: [(None, rnn_size)] * 100
      dec_out = tf.split(dec_out, target_seq_len, axis=0)                        # Shape: [(None, 54)] * 400
    ...
    if architecture == "basic":
      ...
      with vs.variable_scope("basic_rnn_seq2seq"):
        ...
        outputs, self.states = tf.contrib.legacy_seq2seq.rnn_decoder( dec_in, enc_state, cell, loop_function=lf ) # Decoder outputs shape: [(None, rnn_size)] * 100
    ...
    # === Decoder Conv2D Layer === #
    outputs = tf.concat(outputs, axis = 0) # Shape: (100 * None, rnn_size)
    outputs = tf.expand_dims(tf.expand_dims(outputs, axis = 2), axis = 3) # Shape: (100 * None, rnn_size, 1, 1)
    outputs = tf.pad(outputs, [[0, 0], [2, 2], [2, 2], [0, 0]]) # Shape: (100 * None, 36, 5, 1), rnn_size = 32
    outputs = tf.contrib.layers.conv2d(outputs,
                                       num_outputs = self.FRAMES_PER_STEP,
                                       kernel_size = [2, 1],
                                       stride = [2, 2],
                                       activation_fn = tf.nn.relu)   # Shape: (100 * None, 18, 3, 4)
    outputs = tf.transpose(outputs, [0, 3, 1, 2]) # Shape: (100 * None, 4, 18, 3)
  ...
  def get_batch( self, data, actions ):
    ...
    encoder_inputs  = np.zeros((self.batch_size, self.source_seq_len - self.FRAMES_PER_STEP, self.input_size), dtype=float)
    ...
    for i in xrange( self.batch_size ):
      ...
      # Add the data
      encoder_inputs[i, :, 0 : self.input_size]  = data_sel[0 : self.source_seq_len - self.FRAMES_PER_STEP, :]
      decoder_inputs[i, :, 0 : self.input_size]  = data_sel[self.source_seq_len - self.FRAMES_PER_STEP : self.source_seq_len + self.target_seq_len - self.FRAMES_PER_STEP, :]
  ...
  def get_batch_srnn(self, data, action ):
    ...
    encoder_inputs  = np.zeros( (batch_size, source_seq_len - self.FRAMES_PER_STEP, self.input_size), dtype=float )
    ...
    for i in xrange( batch_size ):
      ...
      encoder_inputs[i, :, :]  = data_sel[0 : source_seq_len - self.FRAMES_PER_STEP, :]
      decoder_inputs[i, :, :]  = data_sel[source_seq_len - self.FRAMES_PER_STEP : (source_seq_len + target_seq_len - self.FRAMES_PER_STEP), :]
